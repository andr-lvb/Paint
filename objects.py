...
class Panel:
    ...
    def fill_rect(self, rect, color):
        for r in range(rect.height):
            draw_line_dda(self, Point2D(rect.x, rect.y + r ), Point2D(rect.x + rect.width - 1, rect.y + r), color)
...
class Button:
    ...
    def draw(self, surface):
        ...
        if self.active:
            ...
            draw_line_dda(surface, Point2D(x, y), Point2D(x + width, y), cn.GREEN)
            draw_line_dda(surface, Point2D(x, y + height), Point2D(x + width, y + height), cn.GREEN)
            draw_line_dda(surface, Point2D(x, y), Point2D(x, y + height), cn.GREEN)
            draw_line_dda(surface, Point2D(x + width, y), Point2D(x + width, y + height), cn.GREEN)
